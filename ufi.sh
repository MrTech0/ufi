...
function VerificarFedora {
  if [ "$(uname -r | grep -c .fc3..)" = 0 ] # Compruebo si el SO es Fedora 3x
  then
...
function OptimizarDNF {
	...
	local existe
	existe=$(grep -c fastestmirror $FICHERO)
	
	if [  "$existe" == 0 ] # si no existe la pongo
	then
		echo 'fastestmirror=True' >> $FICHERO
	fi

	existe=$(grep -c max_parallel_downloads $FICHERO)
	if [  "$existe" == 0 ] # si no existe la pongo
	then
		echo 'max_parallel_downloads=10' >> $FICHERO
	fi
}
...
echo "Actualizando sistema y instalando repos básicos"
sleep 3
...
FICHERO="/etc/yum.repos.d/google-chrome.repo"
...
read -rp "¿Cual es el nombre de tu usuario personal? " usuario
...
# Aplicar cambios
function Reiniciar {
  read -rp "¿Quieres reiniciar el sistema? [s/N]: " respuesta
  if [ -n "$respuesta" ]
  then
